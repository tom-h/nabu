...
(function($) {
    ...
    var methods = {
        ...
        /**
         * Allow for filters and filtergroups by flattening the groups on request.
         */
        getFilters: function() {
            var elem = this;
            var groups = elem.jui_filter_rules('getOption', 'filterGroups');
            if (groups.length > 0) {
                var _filters = [];
                for (var i = 0; i < groups.length;i++) {
                    _filters = _filters.concat(groups[i].filters)
                }
                return _filters;
            }
            return elem.data(pluginName)['filters'];
        },
        ...
        getOption: function(opt) {
            var elem = this;
            if (opt === "filters") {
                return elem.jui_filter_rules('getFilters');
            }
    ...
    var create_filters_list = function(container_id) {
        var elem = $("#" + container_id),
            rule_id = parseInt(elem.data(pluginStatus)["rule_id"]),
            filter_groups = elem.jui_filter_rules('getOption', 'filterGroups'),
            filter_groups_len = filter_groups.length,
        ...
        if (filter_groups_len > 0) {
            for(i = 0; i < filter_groups_len; i++) {
                f_html += '<optgroup label="' + filter_groups[i].filter_group_label + '">';

                filters_len = filter_groups[i].filters.length;
                for(j = 0; j < filters_len; j++) {
                    f_html += '<option value="' + filter_groups[i].filters[j].filterName + '">  ' + filter_groups[i].filters[j].filterLabel + '</option>';
                }

                f_html += "</optgroup>"
            }
        } else {
            var filters = elem.jui_filter_rules('getOption', 'filters');
            filters_len = filters.length;
            for(i = 0; i < filters_len; i++) {
    ...
    var create_operators_list = function(container_id, rule_id, filterName) {
        ...
        for(i = 0; i < len; i++) {
            if(oper[i].group !== operators_group) {
                oper_html += '<optgroup label="'+rsc_jui_fr.operator_group_separator+'">';
    ...
    var create_filter_value = function(container_id, rule_id, filterName, operator_type) {
        ...
        if(filter.hasOwnProperty("post_create_callback")) {
            filter["post_create_callback"](elem_filter_value_container);
        }
